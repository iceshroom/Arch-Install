...
echo -e "${red}root(/) locate at：$BOOT_PARTITION"
echo -e "/boot/efi locate at：$INSTALL_PARTITION ${plain}"
sleep 1
...
if [ "$IS_UEFI" -eq '1' ] && [ -n "$isx64" ] ; then
    echo 'grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=GRUB --removable' >> /mnt/set.sh
else
